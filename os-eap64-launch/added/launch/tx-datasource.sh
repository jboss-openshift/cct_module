...
function inject_tx_datasource() {
  ...
  if [ -n "${tx_backend}" ] ; then
    ...
    if [ -z $host ] || [ -z $port ]; then
      ...
      echo_warning "You provided following database mapping (via TX_SERVICE_PREFIX_MAPPING environment variable): $tx_backend. To configure datasources we expect ${service}_SERVICE_HOST and ${service}_SERVICE_PORT to be set."
      echo_warning ""
    ...
    if [ -z $jndi ] || [ -z $username ] || [ -z $password ] || [ -z $database ]; then
      ...
      echo_warning "In order to configure ${db,,} transactional datasource for $prefix service you need to provide following environment variables: ${prefix}_USERNAME, ${prefix}_PASSWORD, ${prefix}_DATABASE."
      echo_warning ""
      echo_warning "Current values:"
      echo_warning ""
