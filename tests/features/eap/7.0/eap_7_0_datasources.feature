...
Feature: EAP Openshift datasources
  ...
  Scenario: check refresh interval is set on the EAP 7.0 datasource when environment variable specified
    When container is started with env
       | variable                                  | value            |
       | DB_SERVICE_PREFIX_MAPPING                 | test-mysql=TEST |
       | TEST_DATABASE                             | kitchensink      |
  ...
  Scenario: check refresh interval is not set on the EAP 7.0 datasource when environment variable is not specified
    When container is started with env
       | variable                                  | value            |
       | DB_SERVICE_PREFIX_MAPPING                 | test-mysql=TEST |
       | TEST_DATABASE                             | kitchensink      |
