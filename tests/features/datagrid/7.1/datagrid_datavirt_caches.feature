...
Feature: Openshift JDG JDV caches
  Scenario: default jdv caches
    ...
    Scenario: default replicated jdv caches
      ...
      Then XML file /opt/datagrid/standalone/configuration/clustered-openshift.xml should contain value ST_StockCache on XPath //*[local-name()='replicated-cache']/@name
      Then XML file /opt/datagrid/standalone/configuration/clustered-openshift.xml should contain value teiid-alias-naming-cache on XPath //*[local-name()='replicated-cache']/@name
