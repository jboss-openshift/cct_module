...
function inject_external_datasources() {
  ...
  if [ -n "$RESOURCES" ]; then
    for prefix in $(echo $RESOURCES | sed "s/,/ /g"); do
      ...
      if [ -z "$name" ]; then
        echo_warning "WARNING! ${prefix}_NAME is missing. Resource $prefix WILL NOT be configured." 
        continue  
      fi      
      ...
      if [ -z "$driver" ]; then
        echo_warning "WARNING! ${prefix}_DRIVER is missing. Resource $prefix WILL NOT be configured." 
        continue
      fi
      ...
      if [ -n "$url" ]; then
        ...
        echo_warning "WARNING! ${prefix}_PROTOCOL, ${prefix}_HOST, ${prefix}_PORT, ${prefix}_DATABASE, or ${prefix}_URL is missing. Resource $prefix WILL NOT be configured." 
        continue
      else
      ...
      if [ -z "$factory" ]; then
        echo_warning "WARNING! ${prefix}_FACTORY is missing. Resource $prefix WILL NOT be configured." 
        continue
      fi
      ...
      if [ -z "$type" ]; then
        echo_warning "WARNING! ${prefix}_TYPE is missing. Resource $prefix WILL NOT be configured." 
        continue
      fi
      ...
      if [ -n "$username" ]; then
        if [ -n "$password" ]; then
          ...
          echo_warning "WARNING! ${prefix}_PASSWORD is missing. Resource $prefix WILL NOT be configured." 
          continue
        fi
      fi
  ...
  for db_backend in ${db_backends[@]}; do
    ...
    if [ "$configurable_db" = true ]; then
      if [ -z $host ] || [ -z $port ]; then
        ...
        echo_warning "You provided following database mapping (via DB_SERVICE_PREFIX_MAPPING environment variable): $db_backend. To configure datasources we expect ${service}_SERVICE_HOST and ${service}_SERVICE_PORT to be set."
        echo_warning ""
        echo_warning "Current values:"
        echo_warning ""
        echo_warning "${service}_SERVICE_HOST: $host"
        echo_warning "${service}_SERVICE_PORT: $port"
        echo_warning ""
      ...
      if [ -z $jndi ] || [ -z $username ] || [ -z $password ] || [ -z $database ]; then
        ...
        echo_warning "In order to configure ${db,,} datasource for $prefix service you need to provide following environment variables: ${prefix}_USERNAME, ${prefix}_PASSWORD, ${prefix}_DATABASE."
        echo_warning ""
        echo_warning "Current values:"
        echo_warning ""
        echo_warning "${prefix}_USERNAME: $username"
        echo_warning "${prefix}_PASSWORD: $password"
        echo_warning "${prefix}_DATABASE: $database"
        echo_warning ""
        echo_warning "WARNING! The ${db,,} datasource for $prefix service WILL NOT be configured."
        continue
      fi
