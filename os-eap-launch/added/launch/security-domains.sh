...
function configure_security_domains() {
  if [ -n "$SECURITY_DOMAINS" ]; then
    for domain_prefix in $(echo $SECURITY_DOMAINS | sed "s/,/ /g"); do
      local security_domain="<security-domain name=\"$domain_prefix\" cache-type=\"default\">"
      ...
      local options=$(compgen -v | grep -s "${domain_prefix}_MODULE_OPTION_")
     
      for option in $(echo $options); do
        option_name=$(echo "${option}" | sed -e "s/${domain_prefix}_MODULE_OPTION_//g")
        option_value=$(find_env $option)

        if [ "$option_name" == "keycloak_config_file" ]; then
          option_name="keycloak-config-file"
        fi

        if [[ "$option_name" == "java_naming"* ]]; then
          option_name="${option_name//_/.}"
        fi

        if [ "$option_name" == "password_stacking" ]; then
          option_name="password-stacking"
        fi
