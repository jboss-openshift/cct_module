...
function configure_infinispan_core() {
  ...
  local containers="<cache-container name=\"clustered\" default-cache=\"$DEFAULT_CACHE\" $cache_container_start $cache_container_statistics>$transport $containersecurity <!-- ##INFINISPAN_CACHE## --> </cache-container><cache-container name=\"security\"/>"
...
function define_datavirt_caches(){
  if [ -n "$DATAVIRT_CACHE_NAMES" ]; then
    for cache_name in $(echo $DATAVIRT_CACHE_NAMES | sed "s/,/ /g"); do
      define_datavirt_cache_variables "${cache_name}" "false"
      define_datavirt_cache_variables "${cache_name}_staging" "false"
